...
public class LeafLoading extends View {
    ...
    public LeafLoading(Context context) {
        ...
        mLeftMargin = UiUtils.dipToPx(context, LEFT_MARGIN);//18
        mRightMargin = UiUtils.dipToPx(context, RIGHT_MARGIN);//50
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mOrangeRectF = new RectF(mLeftMargin + mArcRadius, mLeftMargin, mLeftMargin + 2 * mArcRadius, mTotalHeight - mLeftMargin);
    ...
    private void drawProgressAndLeafs(Canvas canvas) {
        ...
        //当前所在绘制进度条的位置=进度条宽度*当前进度/总进度=占总进度的百分比
        mCurrentProgressPosition = mProgressWidth * mProgress / TOTAL_PROGRESS;
        // 即当前位置在图中所示1范围内
        if (mCurrentProgressPosition < mArcRadius) {//当前进度小于弧形半径
            ...
            //绘制Orange RECT
            mOrangeRectF.left = mArcRightLocation;//arc的右上角的x坐标，也是矩形x坐标的起始点
            mOrangeRectF.right = mCurrentProgressPosition;//当前进度
    ...
    private void drawLeafs(Canvas canvas) {
        ...
        for (int i = 0; i < mLeafInfos.size(); i++) {
            ...
            if (currentTime > leaf.startTime && leaf.startTime != 0) {
                //绘制叶子--根据叶子的类型和当前的时间得出叶子的（x,y)
                getLeafLocation(leaf, currentTime);//根据时间计算旋转角度
                canvas.save();
                Matrix matrix = new Matrix();
                float transX = mLeftMargin + leaf.x;
                float transY = mLeftMargin + leaf.y;
                matrix.postTranslate(transX, transY);//移动叶子
                //通过时间关联旋转角度，则可以直接通过修改LEAF_ROTATE_TIME调节叶子旋转快慢
                float rotateFranction = ((currentTime - leaf.startTime) % mLeafRotateTime) / (float) mLeafRotateTime;
                int angle = (int) (rotateFranction * 360);
    ...
    private void getLeafLocation(Leaf leaf, long currentTime) {
        long intervalTime = currentTime - leaf.startTime;//时间间隔
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.LeafLoading);
        mProgress = typedArray.getInteger(R.styleable.LeafLoading_progress, 0);
        mMiddleAmplitude=typedArray.getInteger(R.styleable.LeafLoading_middleAmplitude,0);
        mLeafFloatTime=typedArray.getInteger(R.styleable.LeafLoading_leafRotateTime,0);
        mLeafRotateTime=typedArray.getInteger(R.styleable.LeafLoading_leafRotateTime,0);
        mAmplitudeDisparity=typedArray.getInteger(R.styleable.LeafLoading_mplitudeDiaparity,0);
        typedArray.recycle();
    }
